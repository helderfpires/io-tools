 ...
 *
 */
package com.gc.iotools.fmt.detector.droid.signatureFile;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

import com.gc.iotools.fmt.detector.droid.MessageDisplay;
...
public class SubSequence extends SimpleElement {
	...
	public int getNumAlternativeFragments(final boolean leftFrag,
		...
		if (leftFrag) {
			return ((ArrayList) this.orderedLeftFragments.get(thePosition - 1))
					.size();
		} else {
			return ((ArrayList) this.orderedRightFragments.get(thePosition - 1))
					.size();
		}
	}
	...
	public boolean isFoundAfterFileMarker(final ByteReader targetFile,
		...
		startPosInFile = startPosInFile
				+ (searchDirection * this.getMinSeqOffset());
		...
		// relevant fragments
		startPosInFile = startPosInFile
				+ (searchDirection * this.getMinFragLength());
		final int numSeqBytes = this.getNumBytes();
				...
				if (this.byteSequence[byteLoopEnd] != lastByte) {
					...
					for (int iByte = byteLoopStart; (!missMatchFound)
						...
						missMatchFound = (this.byteSequence[iByte] != targetFile
								.getByte(startPosInFile + iByte - byteLoopStart));
					}
					if (!missMatchFound) { // subsequence was found at position
						...
						if (reverseOrder) {
							final long[] rightFragEndArray = bytePosForRightFragments(
									targetFile, startPosInFile + 1, targetFile
											.getFileMarker(), 1, 0, bigEndian);
	...
	public boolean isFoundAtStartOfFile(final ByteReader targetFile,
		...
		try {
			...
			if (this.reference.startsWith("Indirect")) {
				try {
					final int indirectOffset = this
							.getIndirectOffset(targetFile);
			...
			if (subseqFound) {
				...
				if (offsetRange > 0) {
					...
					if (searchDirection > 1) {
						System.arraycopy(newStartPosInFile, 0, startPosInFile,
								0, numOptions);
					} else {
						// reverse order copy
						for (int i = 0; i < numOptions; i++) {
	...
	public void prepareSeqFragments() {
		...
		for (int i = 0; i < this.leftFragments.size(); i++) {
			final int currentPosition = this.getRawLeftFragment(i)
					.getPosition();
		...
		for (int i = 0; i < this.rightFragments.size(); i++) {
			final int currentPosition = this.getRawRightFragment(i)
					.getPosition();
			if (currentPosition > numFrags) {
				numFrags = currentPosition;
			}
	...
	public void setSequence(final String seq) {
		...
		if (2 * seqLength != seq.length()) {
			System.out.println("A problem - sequence of odd length was found: "
					+ seq);
		}
	...
	private long[] bytePosForLeftFragments(final ByteReader targetFile,
		...
		int posLoopStart = 1;
		final int numFragPos = this.getNumFragmentPositions(leftFrag);
		if (searchDirection == 1) {
		...
		for (int iFragPos = posLoopStart; (!seqNotFound)
			...
			final int numAltFrags = this.getNumAlternativeFragments(leftFrag,
					iFragPos);
			...
			for (int iOption = 0; iOption < numOptions; iOption++) {
				...
				for (int iAlt = 0; iAlt < numAltFrags; iAlt++) {
					long tempFragEnd;
					if (searchDirection == 1) {
						tempFragEnd = this.endBytePosForSeqFrag(targetFile,
	...
	private long endBytePosForSeqFrag(final ByteReader targetFile,
		...
		if (leftFrag && (searchDirection == -1)) {
			minOffset = this.getFragment(leftFrag, fragPos, fragIndex)
					.getMinOffset();
			maxOffset = this.getFragment(leftFrag, fragPos, fragIndex)
					.getMaxOffset();
			...
			minOffset = this.getFragment(leftFrag, fragPos + 1, 0)
					.getMinOffset();
			maxOffset = this.getFragment(leftFrag, fragPos + 1, 0)
					.getMaxOffset();
		} else {
			minOffset = 0;
			maxOffset = 0;
		...
		boolean subSeqFound = false;
		while ((!subSeqFound)
				&& ((searchDirectionL) * (lastStartPosInFile - startPosInFile) >= 0L)) {
			boolean missMatchFound = false;
			if (searchDirection == -1) {
			} else {
			}
			final SideFragment fragment = this.getFragment(leftFrag, fragPos,
					fragIndex);
	...
	private int getIndirectOffset(final ByteReader targetFile) {
		...
		long offsetLocation = this.getByteSequence()
				.getIndirectOffsetLocation();

		if (this.getByteSequence().getReference().endsWith("EOFoffset")) {
			offsetLocation = targetFile.getNumBytes() - offsetLocation - 1;
		}

		final int offsetLength = this.getByteSequence()
				.getIndirectOffsetLength();
