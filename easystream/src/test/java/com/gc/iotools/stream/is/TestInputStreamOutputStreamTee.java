...
public class TestInputStreamOutputStreamTee {
	...
	/*
	 * Test standard use case, no frills. Data are read from the InputStream
	 * and they're copied on the destination stream
	 */
	@org.junit.Test
	public void testReadWrite() throws Exception {
		...
		teeStream.close();
		assertArrayEquals("array are equals", testBytes,
				destination.toByteArray());
	}

	/*
	 * Method close() is invoked before the data is read from the InputStream
	 * the data is still copied to the OutputStream(s)
	 */
	@org.junit.Test
	public void testSuddenClose() throws Exception {
		...
		final InputStream teeStream = new TeeInputStreamOutputStream(istream,
				bos);
		// close but no data read from the istream
		teeStream.close();
		final byte[] result = bos.toByteArray();
