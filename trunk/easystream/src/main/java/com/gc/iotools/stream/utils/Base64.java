 ...
 * I've got Base64 set the opposite way so that you tell it _not_ to add line
 * feeds since I always thought line feeds were required -- I was wrong, but
 * I'm reluctant to change the default behavior now. Sorry.
 * </p>
 * <p>
 ...
 * some similar helper methods to be more efficient with memory by not
 * returning a String but just a byte array.</li>
 ...
 * streams, there is a possiblity of an java.io.IOException being thrown.
 * After some discussion and thought, I've changed the behavior of the methods
 * to throw java.io.IOExceptions rather than return null if ever there's an
 * error. I think this is more appropriate, though it will require some
 * changes to your code. Sorry, it should have been done this way to begin
 * with.</li>
 * <li>
 ...
 * that's being decoded is gzip-compressed and will decompress it
 * automatically. Generally things are cleaner. You'll probably have to change
 * some method calls that you were making to support the new options format (
...
public class Base64 {
					...
					for (i = 0; i < 4; i++) {
						...
						do {
							b = this.in.read();
								...
								&& (this.decodabet[b & 0x7f] <= WHITE_SPACE_ENC));
	...
	public static class OutputStream extends java.io.FilterOutputStream {
		...
		/**
		 * Suspends encoding of the stream. May be helpful if you need to
		 ...
		 * Base64 notation. When encoding, bytes are buffered three at a time
		 * before the output stream actually gets a write() call. When
	 ...
	 * that data encoded this way is <em>not</em> officially valid Base64, or
	 * at the very least should not be called Base64 without also specifying
	 ...
	 * generally a recommended method, although it is used internally as part
	 * of the decoding process. Special case: if len = 0, an empty array is
	 * returned. Still, if you need more speed and reduced memory footprint
	 * (and aren't gzipping), consider this method.
	 * 
	 * @param source
	...
	public static void decodeToFile(final String dataToDecode,
		...
		try {
			bos = new Base64.OutputStream(new java.io.FileOutputStream(
	...
	private static byte[] encode3to4(final byte[] b4,
			final byte[] threeBytes, final int numSigBytes, final int options) {
	...
	private static byte[] encode3to4(final byte[] source,
			final int srcOffset, final int numSigBytes,
			final byte[] destination, final int destOffset, final int options) {
	 ...
	 * byte array instead of instantiating a String. This is more efficient if
	 ...
	 */
	public static byte[] encodeBytesToBytes(final byte[] source,
			final int off, final int len, final int options)
			throws java.io.IOException {
	...
	public static String encodeFromFile(final String filename)
		...
		try {
			...
			final byte[] buffer = new byte[Math.max(
					(int) (file.length() * 1.4), 40)]; // Need max() for math
			// on
			// small files (v2.2.1)
			int length = 0;
			int numBytes = 0;
