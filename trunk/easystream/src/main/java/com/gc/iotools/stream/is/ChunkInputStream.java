 ...
 * bytes equal to <code>startMarker</code> is found. When a sequence of bytes
 * equals to <code>endMarker</code> is found the stream hide the bytes until a
 * new <code>startMarker</code> is found.
 * </p>
 ...
 * above.</li>
 * <li><code>automaticFetch=false</code> Each chunk need to be fetched
 * invoking explicitly a <code>{@link #fetchNextChunk()}</code> methods. The
 * stream is initially in an EOF state and
 * <code>{@link #fetchNextChunk()}</code> must be called at first. At this
 * point all the bytes of the stream are shown until an <code>endMarker</code>
 * is found. At this point the ChunkInputStream is in an EOF state until a
 * <code>{@link #fetchNextChunk()}</code> is invoked.</li>
 * <li><code>automaticFetch=false</code> and <code>startMarker=null</code> It
...
public final class ChunkInputStream extends InputStream {
	 ...
	 *            <code>startMarker</code> is found. If this parameter is set
	 *            to <code>null</code> the stream is made available until the
	 ...
	 *            until a <code>fetchNextChunk()</code> is performed.
	 * @param showMarkers
	 *            if set to <code>true</code> start and end markers are shown
	 *            in the outer stream.
