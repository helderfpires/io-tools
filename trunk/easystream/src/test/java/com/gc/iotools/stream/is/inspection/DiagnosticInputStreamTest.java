...
public class DiagnosticInputStreamTest {
	...
	public void testMethodCalledAfterClose() throws Exception {
		ByteArrayInputStream bais = new ByteArrayInputStream("1234"
				.getBytes());
		DiagnosticInputStream<InputStream> diagIs = new DiagnosticInputStream<InputStream>(
				bais);
		diagIs.close();
		...
		assertTrue("isMethodCalledAfterClose ", diagIs
				.isMethodCalledAfterClose());
		final String statusString = diagIs.getStatusMessage();
		assertNotNull("Status string", statusString);
		assertTrue("Status string contains error", statusString
				.contains("ALREADY_CLOSED"));
		diagIs.clearInstanceWarnings();
	}
	...
	public static void tearDown() throws Exception {
		System.gc();
		Thread.sleep(2000);
		String[] errors = DiagnosticInputStream.getFinalizationErrors();
		assertEquals("number of errors", 1, errors.length);
		assertTrue("Error not closed [" + errors[0] + "]", errors[0]
				.contains("NOT_CLOSED"));
