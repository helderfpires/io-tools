...
public class DiskBufferedInputStream extends InputStream {
	...
	private final InputStream diskBuffered = null;
	private InputStream memoryBuffered = null;
	...
	public synchronized void mark(final int readlimit) {
		final boolean markForever = readlimit < 0;
		final int effectiveReadLimit = (markForever ? this.threshold : Math
				.min(readlimit, this.threshold));

		if (this.diskBuffered == null) {
			if (this.original.markSupported()) {
				// original supports marking and not disk buffering
				this.memoryBuffered = this.original;
			} else {
				this.memoryBuffered = new BufferedInputStream(this.original);
			}
			this.memoryBuffered.mark(effectiveReadLimit);
		} else {
			this.memoryBuffered = new BufferedInputStream(this.original);
		}
