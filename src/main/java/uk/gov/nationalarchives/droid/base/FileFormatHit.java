...
public class FileFormatHit extends SimpleElement {
    String myHitWarning = "";
    int myHitType;
    DroidFileFormat myHitFileFormat;
     ...
     */
    public FileFormatHit(DroidFileFormat theFileFormat, int theType, boolean theSpecificity, String theWarning) {
        myHitFileFormat = theFileFormat;
        if (theType == DroidConstants.HIT_TYPE_POSITIVE_GENERIC_OR_SPECIFIC) {
            if (theSpecificity) {
                myHitType = DroidConstants.HIT_TYPE_POSITIVE_SPECIFIC;
            } else {
                myHitType = DroidConstants.HIT_TYPE_POSITIVE_GENERIC;
            }
        } else {
            myHitType = theType;
        }
     ...
     */
    public DroidFileFormat getFileFormat() {
    ...
    public String getHitTypeVerbose() {
        String theHitType = "";
        if (myHitType == DroidConstants.HIT_TYPE_POSITIVE_GENERIC) {
            theHitType = DroidConstants.HIT_TYPE_POSITIVE_GENERIC_TEXT;
        } else if (myHitType == DroidConstants.HIT_TYPE_POSITIVE_SPECIFIC) {
            theHitType = DroidConstants.HIT_TYPE_POSITIVE_SPECIFIC_TEXT;
        } else if (myHitType == DroidConstants.HIT_TYPE_TENTATIVE) {
            theHitType = DroidConstants.HIT_TYPE_TENTATIVE_TEXT;
        } else if (myHitType == DroidConstants.HIT_TYPE_POSITIVE_GENERIC_OR_SPECIFIC) {
    ...
    public boolean isSpecific() {
        if (myHitType == DroidConstants.HIT_TYPE_POSITIVE_SPECIFIC) {
            return true;
        } else {
            return false;
        }
    ...
    public void setStatus(String value) {
        ...
        if (value.equals(DroidConstants.HIT_TYPE_POSITIVE_GENERIC_TEXT)) {
            myHitType = DroidConstants.HIT_TYPE_POSITIVE_GENERIC;
        } else if (value.equals(DroidConstants.HIT_TYPE_POSITIVE_SPECIFIC_TEXT)) {
            myHitType = DroidConstants.HIT_TYPE_POSITIVE_SPECIFIC;
        } else if (value.equals(DroidConstants.HIT_TYPE_TENTATIVE_TEXT)) {
            myHitType = DroidConstants.HIT_TYPE_TENTATIVE;
        } else if (value.equals(DroidConstants.HIT_TYPE_POSITIVE_GENERIC_OR_SPECIFIC_TEXT)) {
            myHitType = DroidConstants.HIT_TYPE_POSITIVE_GENERIC_OR_SPECIFIC;
        } else {
    ...
    public void setName(String value) {
        ...
        if (myHitFileFormat == null) {
            myHitFileFormat = new DroidFileFormat();
        }
